"""Página de análisis detallado de Facebook."""
...
def renderizar():
    """Renderizar la página de Facebook."""
    st.title("📘 Facebook")
    st.markdown("Análisis detallado de páginas de Facebook.")

    with st.form("form_facebook"):
        usuario = st.text_input(
            "Nombre de página o Page ID",
            placeholder="ej: mi_marca o 123456789",
        )
    ...
    if not analizar or not usuario:
        st.info("Ingresa el nombre de una página o Page ID para comenzar.")
        return

    collector = FacebookCollector()
    calculador = CalculadorMetricas()

    with st.spinner("Obteniendo datos de Facebook..."):
    ...
    for i, pub in enumerate(top_pubs, 1):
        ...
        with st.expander(
            ...
            f"👍 {pub.likes:,} 💬 {pub.comentarios:,} 🔄 {pub.compartidos:,}"
        ):
            st.write(pub.texto[:200] if pub.texto else "Sin descripción")
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("Reacciones", f"{pub.likes:,}")
